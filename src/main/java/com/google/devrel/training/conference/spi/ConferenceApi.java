...
public class ConferenceApi {
	...
	public Profile saveProfile(final User user, ProfileForm profile_form) throws UnauthorizedException {
		...
		if(profile_form.getDisplayName()!=null) displayName = profile_form.getDisplayName();
	 ...
	 */
	@ApiMethod(name = "createConference", path = "createConference", httpMethod = HttpMethod.POST)
		...
		WrappedBoolean result = ofy().transact(new Work <WrappedBoolean>() { 
			public WrappedBoolean run () { 
				...
				try {
					...
					Key<Conference> conferenceKey = Key.create(websafeConferenceKey);
					if(conferenceKey==null) throw new ForbiddenException("Forbidden exception");
	...
	public Collection<Conference> getConferencesToAttend(final User user)
		...
		List<Conference> conferencesToAttend = new  ArrayList<Conference>();
		for(String s: keyStringsToAttend){
			conferencesToAttend.add((Conference) ofy().load().key(Key.create(s)).now());
		}
	...
	public WrappedBoolean unregisterFromConference(final User user, @Named("websafeConferenceKey") final String websafeConferenceKey) throws UnauthorizedException, NotFoundException, ForbiddenException, ConflictException {
		...
		WrappedBoolean result = ofy().transact(new Work <WrappedBoolean>() { 
			public WrappedBoolean run () { 
				...
				try {
					...
					if (!profile.getConferenceKeysToAttend().contains(
							websafeConferenceKey)) {
						return new WrappedBoolean (false, "Not registered for the conference!");
					} 
					 else {
		...
		if (!result.getResult()) {
			...
			else if (result.getReason() == "Not registered for the conference!") {
